...
def scrape_rain_headless(target_date):
    """
    ดึงข้อมูลแบบ Headless (ไม่เปิดจอ) + บังคับโชว์ครบทุกเขต
    """
    driver = None
    df = None
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            # Setup Headless Driver
            options = webdriver.ChromeOptions()
            options.add_argument("--headless=new") # ✅ ไม่เปิดหน้าจอ
            ...
            # ✅ แก้ไขวันที่: ใช้ปี ค.ศ. (2025) ไม่ใช่ พ.ศ.
            ...
            # 5. [สำคัญ] บังคับให้แสดง 100 แถว
            # รอให้ตารางโหลดก่อน
            WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.TAG_NAME, "tbody")))
            
            try:
                # หา Dropdown 'Show entries' (name มักจะลงท้ายด้วย _length)
                length_select_elem = driver.find_element(By.XPATH, "//select[contains(@name, 'length')]")
                select_obj = Select(length_select_elem)
                
                # เลือกค่า '100' หรือค่าสูงสุดที่มี
                try:
                    select_obj.select_by_value("100")
                except:
                    select_obj.select_by_index(len(select_obj.options)-1)
                
                logging.info("   Changed view to 100 rows.")
                
                # [สำคัญ] รอให้ตารางขยายจริง (เช็คว่าแถวเพิ่มขึ้นเกิน 15 แถว)
                WebDriverWait(driver, 10).until(
                    lambda d: len(d.find_elements(By.XPATH, "//table//tbody//tr")) > 15
                )
                
            except Exception as e:
                logging.warning(f"   ⚠️ Could not expand table (might handle partial data): {e}")
            ...
            if dfs:
                df = dfs[0]
                # เช็คว่าได้ข้อมูลมาครบไหม (ต้อง > 40 เขต)
                if len(df) > 40:
                    logging.info(f"   ✅ Successfully scraped {len(df)} rows")
                    break
                else:
...
def upload_to_supabase(df, target_date, district_map):
    ...
    if records:
        ...
        try:
            supabase.table("rain_logs").insert(data).execute()
            logging.info(f"   💾 Saved {len(data)} districts to Database")
        except Exception as e:
            logging.error(f"   ❌ DB Insert Error: {e}")
    else:
        logging.warning("   ⚠️ No valid records to upload.")
...
def main():
    ...
    # เริ่มดึงตั้งแต่วันถัดไป
    start_date = last_date + timedelta(days=1)
    # เอาถึงแค่วันเมื่อวาน (Today - 1)
    end_date = date.today() - timedelta(days=1) 
